...
RSpec.feature "Transaction index page", type: :feature do
  ...
  let!(:transaction1) { Operation.create(name: 'Groceries', amount: 50, user: user, groups: [group1]) }
  let!(:transaction2) { Operation.create(name: 'Clothes', amount: 30, user: user, groups: [group2]) }
