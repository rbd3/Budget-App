...
RSpec.describe Operation, type: :model do
  ...
  let(:group) { Group.create(name: 'Food', user: user, icon: 'food_icon.png') }

  it 'is valid with valid attributes' do
    operation = Operation.new(name: 'Transaction', amount: 50.0, user: user)
    operation.groups << group
    expect(operation).to be_valid
  end

  it 'is not valid without a name' do
    operation = Operation.new(amount: 50.0, user: user)
    operation.groups << group
    operation.name = nil
  ...
  it 'is not valid without an amount' do
    operation = Operation.new(name: 'Transaction', user: user)
    operation.groups << group
  ...
  it 'is not valid with a non-positive amount' do
    operation = Operation.new(name: 'Transaction', amount: -10.0, user: user)
    operation.groups << group
    expect(operation).to_not be_valid
  end
